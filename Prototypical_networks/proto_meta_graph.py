...
from model_graph import GraphFeatureExtractor
...
class PrototypicalNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.feature_extractor = GraphFeatureExtractor(52, 8)
    ...
    def forward(self, input_batch, device = torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        scaler = StandardScaler()
        support_features, support_target, query_features, query_target = input_batch
        ...
        support_reactant_graph = self.feature_extractor(gs_react.to(device))
        support_ligand_graph = self.feature_extractor(gs_lig.to(device))
        support_solvent_graph = self.feature_extractor(gs_solv.to(device))
        ...
        support_graph_features = torch.cat([support_reactant_graph, support_ligand_graph, support_solvent_graph, support_condition], axis=1)
        self.fc = self.fc.double()
        support_features_flat = self.fc(support_graph_features)
        ...
        query_reactant_graph = self.feature_extractor(gs_react_query.to(device))
        query_ligand_graph = self.feature_extractor(gs_lig_query.to(device))
        query_solvent_graph = self.feature_extractor(gs_solv_query.to(device))
        ...
        query_graph_features = torch.cat([query_reactant_graph, query_ligand_graph, query_solvent_graph, query_condition], axis=1)
        query_features_flat = self.fc(query_graph_features)
